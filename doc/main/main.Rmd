...
p <- 10 # number of predictors (excluding intercept)
...
Using the validation sets we may compare the performance of the regression procedures numerically, using both the tuning parameters corresponding to the least testing error, $\lambda_{\text{min}}$, and the one-standard-error rule, $\lambda_{1\text{se}}$.
```{r}
...
for (j in 1:p) {
  plot(mod.ahb.cv$model.fit, 
       ...
       plot.type = 'lines', 
       plot.stat = 'coef')
  abline(v = mod.ahb.cv$lambda.1se, col = 'gray50', lty = 'dotted')
  if (beta[j] != 0) {
