...
subtitle: "Exploration and Expansion of Nonparametric Regression via `HierBasis`"
author: 
  David Fleischer
  Annik Gougeon
...
for ${\bm x}_i \sim \mathcal N_p(0, \mathbb I_p)$ and $\epsilon_i \sim \mathcal N(0, \sigma^2)$ such that the noise variance $\sigma^2$ satisfies
$$
...
Plotted below are a set of graphs illustrating the coefficient paths (excluding the intercept), with each predictor presented in a separate plot. Plotted above each figure is the number of active hierarchical basis features for the corresponding value of $\lambda$, with the diffrent lines corresponding to the different hierarchical basis features 
...
We repeat the figures from the linear model variable selection section for the additive modelling environment. First, we investigate `HierBasis`'s ability to select the correct features from the design matrix via the coefficient plots as a function of $\lambda$ 
```{r, echo = F, fig.align = 'center', fig.height = 3, fig.width = 3.5, fig.show = 'hold'}
for (j in 1:p) {
  plot(mod.ahb.cv$model.fit, 
       pred.idx = j, 
...
The `HierBasis` documentation references a mixing parameter $\alpha$ which controls the degree to which the two penalty terms are balanced according to
$$
  \alpha \lambda \sum^p_{j = 1} \Omega_j\left(\bbeta_j\right) + (1 - \alpha)\frac{\lambda^2}{\sqrt{n}} \sum^p_{j = 1} \left\lVert \Psi^{(j)}_K\bbeta_j \right\rVert^2_2.
$$

That is, $\alpha$ controls the balance between the hierarchical penalty and the sparsity penalty. A natural question is to ask what affect does varying $\alpha$ have on variable selection? To address this we fit an additive `HierBasis` model specifying $\alpha \in \left\{ \frac{i}{n_\alpha}\right\}^{n_\alpha}_{i = 1}$. For this simulation study we consider the same additive model as before
$$
  y_i = 2 + 5f_1(x_{i1}) + 3f_2(x_{i2}) + 4f_3(x_{i3}) + 6f_4(x_{i4}) + \epsilon_i,
$$

with $f_1, f_2, f_3, f_4$ the same as in the previous sections. We fit the additive `HierBasis` model specifying $\texttt{nbasis} = 10$ and selecting $\lambda_{\text{1se}}$ as the tuning parameter corresponding to the fitted values of $\widehat\bbeta$. For each value of $\alpha$ we check whether the active predictors $(X_1, X_2, X_3, X_4)$ are correctly identified as being active via the fitted $\widehat\bbeta$, i.e., we check whether $\widehat\bbeta_1, ..., \widehat\bbeta_4 \neq {\bf 0}$ and whether $\widehat\bbeta_5, ..., \widehat\bbeta_9 = {\bf 0}$. We perform this process $\texttt{nsims} = 30$ times in order to get an estimate of the rates of variable selection/rejection, as well as computing the corresponding type I and type II errors.
...
\begin{align*}
  p_{00, j} &= \widehat{\mathbb P}\left( \widehat\bbeta_j = {\bf 0} \mid X_j \text{ is inactive} \right) \quad \text{(True Negative)} \\
  p_{10, j} &= \widehat{\mathbb P}\left( \widehat\bbeta_j \neq {\bf 0} \mid X_j \text{ is inactive} \right) \quad \text{(Type I)} \\
  p_{01, j} &= \widehat{\mathbb P}\left( \widehat\bbeta_j = {\bf 0} \mid X_j \text{ is active} \right) \quad \text{(Type II)} \\
  p_{11, j} &= \widehat{\mathbb P}\left( \widehat\bbeta_j \neq {\bf 0} \mid X_j \text{ is active} \right) \quad \text{(True Positive)}, 
\end{align*}

such that
$$
  \widehat{\mathbb P}\left( \widehat\bbeta_j = {\bf 0} \mid X_j \text{ is (in)active} \right) = \frac{1}{\texttt{nsims}} \sum^{\texttt{nsims}}_{l = 1} {\mathbf 1}_{\left\{ \widehat\bbeta_j = {\bf 0} \mid X_j \text{ is (in)active} \right\}}
$$

so that the estimates of the classification rates are
$$
  p_{n_1 n_2} = \frac{1}{p} \sum^p_{j = 1} p_{n_1n_2, j}, \quad n_1, n_2 = 0, 1.
$$

Thus,
\begin{align*}
  \widehat{\mathbb P}\left( \text{correct variable selection/rejective} \right) &= p_{00} + p_{11} \\
  \widehat{\mathbb P}\left( \text{incorrect variable selection/rejection} \right) &= p_{01} + p_{10} \\
  \widehat{\mathbb P}\left(\text{type I error}\right) &= p_{10} \\
  \widehat{\mathbb P}\left(\text{type II error}\right) &= p_{01}.
\end{align*}

and
$$
  \widehat{\text{Var}}\left(p_{n_1n_2}\right) = \frac{1}{p^2 \cdot \texttt{nsims}} \sum^\texttt{nsims}_{l = 1} \sum^p_{j = 1} p_{n_1n_2, j} \cdot (1 - p_{n_1n_2, j}), \quad n_1, n_2 = 0, 1.
$$
...
Plotted below are the empirical selection/rejection probabilities (as defined above) as a function of $\alpha$, with the corresponding standard deviations above and below the mean probabilities.

```{r, echo = F, fig.align = 'center', fig.show = 'hold', fig.height = 6, fig.width = 6.5}
par(mfrow = c(2, 2))
...
From these figures we find little evidence that manipulating $\alpha$ has a material impact on overall variable selection/rejection rates. That is, the probability that a feature is correctly identified as being either a part or not a part of our additive model appears to be independent of $\alpha$, with perhaps a slight decrease in classification rates as $\alpha \to 1$. One way to interpret this slight decrease in the feature detection performance of the `HierBasis` model is by noting that as $\alpha \to 1$ we find our penalty retains only the hierarchical term
$$
  \lambda \sum^p_{j = 1} \Omega_j\left(\bbeta_j\right) = \frac{\lambda}{\sqrt{n}} \sum^p_{j = 1} \sum^\texttt{K}_{k = 1} w_k \left\lVert \Psi^{(j)}_{k:K} \bbeta_{j,k:K} \right\rVert_2
$$

and drops the second additional sparsity-inducing penalty
$$
  \frac{\lambda^2}{\sqrt{n}} \sum^p_{j = 1} \left\lVert \Psi^{(j)}_K \bbeta_j \right\rVert_2
$$

The effect of dropping this secondary sparsity-inducing term is made more apparent by the type I and type II figures. As $\alpha\to 1$ and this additional sparsity-inducing term vanishes we expect estimates $\widehat\bbeta$ that are hierarchically sparse (i.e. only the first few orders of the basis expansion are expected to be estimated as nonzero), but are nonsparse across the $p$ input features $\X = (X_1, ..., X_p)$. That is, as $\alpha\to 1$ we expect to see more features to be estimated as nonzero/influencial, leading a greater occurence of type I errors. 

Likewise, as $\alpha \to 0$, we find that the hierarchical-sparsity term vanishes while the additional sparsity-inducing term is able to exert its full force on the penalty. As a result we expect the solutions $\widehat\bbeta$ to be sparse with respect to the input features $\X$, but nonsparse with respect to a hierarchical structure (i.e. higher degrees of the basis expansion will be estimated as nonzero). So, as $\alpha \to 0$, we expect to see fewer features selected (but each selected feature is expected to have a higher-order basis expansion), leading to a greater occurence of type II errors.

The above simulation agrees with this interpretation of our mixing parameter $\alpha$, as can be seen from the increasing rate of type I errors and decreasing rate of type II errors as a function of $\alpha$.
...
\Omega_j(\bbeta_j) = \frac{1}{\sqrt{n}} \sum^K_{k = 1} w_k \left\lVert \Psi^{(j)}_{k:K} \bbeta_{j,k:K} \right\rVert_2,
$$
where $w_k = k^m - (k - 1)^m$ are penalization weights for the $k^\text{th}$-order basis estimator. According to @haris2016nonparametric, the tuning parameter $m$ is similar to the number of bounded derivatives employed in the simple project estimator (@cencov1962estimation) and recommends the use of $m = 2$ or 3 in order to achieve good results, and that $m = 3$ is analogous to applying a cubic spline for smoothing. We explore this suggestion, and other values of $m$, below.
...
for ${\bm x}_i$, $\epsilon_i$, and $\sigma^2$ as defined in the previous exploration of `HierBasis`'s performance in linear models. We will characterize the optimal value(s) of $m$ by minimizing the MSE of a validation set (drawn from the same distribution as the training set). First, we generate our predictors and responses.
```{r}
#===== parameters =====#
set.seed(400)
n <- 1000 # number of observations
...
Now, we fit multiple `HierBasis` models with varying values of $m$ for fixed value of $\lambda$ to be $\lambda = 0.25$.
```{r}
...
```{r, echo = F, fig.align = 'center', fig.height = 4, fig.width = 5}
plot(mse.valid[,1] ~ m.vec, 
     log = 'y', main = "Test Error",
     xlab = "m", ylab = "MSE", 
     type = 'l', lwd = 1.5)
```
...
Once again, we note that the optimal $m$ within the suggested range of $m = 2$ or 3, as noted in @haris2016nonparametric. The analysis above indicates that the optimal $m$ value in terms of minimizing the MSE is found at $m = 3$ for this particular additive model. 
